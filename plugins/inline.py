...
from utils import get_search_results, is_user_active
...
async def answer(bot, query):
    ...
    # Referral check
    if not await is_user_active(query.from_user.id):
        await query.answer(
            results=[],
            cache_time=0,
            switch_pm_text='Please invite required users using your referral link to use the bot.',
            switch_pm_parameter="referral",
        )
        return
    ...
    for file in files:
        results.append(
            InlineQueryResultCachedDocument(
                title=getattr(file, "file_name", "File"),
                document_file_id=file.file_id,
                caption=getattr(file, "caption", "") or "",
                description=f'Size: {size_formatter(getattr(file, "file_size", 0))}\nType: {getattr(file, "file_type", "")}',
